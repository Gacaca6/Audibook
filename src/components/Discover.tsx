  ...
  return (
    <div className="flex flex-col gap-4" id="discover-container">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <div className="flex-1 flex items-center gap-2 bg-white border-2 border-gray-200 rounded-2xl px-3.5 focus-within:border-[#6D4AFF] transition-colors">
        ...
        <button
          ...
          className="px-4 bg-[#6D4AFF] border-b-4 border-[#5433E0] active:border-b-0 active:translate-y-[4px] text-white rounded-2xl font-sans font-black text-sm transition-all disabled:opacity-50 disabled:border-b-4 disabled:translate-y-0"
        >
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-4">
            {SUGGESTIONS.map((s) => (
              <button
                key={s}
                onClick={() => runSearch(s)}
                className="px-3 py-2 bg-[#E0F7F4] border border-[#00B3A4]/20 text-[#00B3A4] rounded-xl font-sans font-black text-[11px] active:scale-95 transition-all"
              >
                {s}
              </button>
            ))}
          ...
          <AnimatePresence>
            {results.map((r) => {
              ...
              return (
                ...
                >
                  {/* Cover */}
                  <div className="w-14 h-[76px] rounded-xl overflow-hidden bg-gradient-to-br from-[#9B85FF] to-[#6D4AFF] shrink-0 flex items-center justify-center relative">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col">
                    ...
                    <div className="mt-auto pt-1.5 flex items-center gap-2">
                      <span className="font-mono text-[9px] text-slate-400 bg-slate-50 border border-slate-100 rounded-full px-2 py-0.5">
                        🎧 {formatListens(r.downloads)} listens
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center">
                    {onShelf ? (
                      <span className="flex items-center gap-1 px-3 py-2 bg-[#6D4AFF]/10 border border-[#6D4AFF]/20 text-[#6D4AFF] rounded-xl font-sans font-black text-[11px]">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-2.5 bg-[#00B3A4] border-b-4 border-[#00907F] active:border-b-0 active:translate-y-[4px] text-white rounded-xl font-sans font-black text-[11px] transition-all disabled:opacity-60"
                      >
                        {isAdding ? (
